...
func TestPriorityScheduler(t *testing.T) {
	...
	exec1 := &kernel.Execution{ID: "exec-1", Priority: kernel.PriorityClassHigh}
	exec2 := &kernel.Execution{ID: "exec-2", Priority: kernel.PriorityClassNormal}
	exec3 := &kernel.Execution{ID: "exec-3", Priority: kernel.PriorityClassCritical}
