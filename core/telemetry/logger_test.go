...
func TestPlatformLoggerErrorExpansion(t *testing.T) {
	...
	platErr := rtErrors.NewError(rtErrors.ErrCategoryValidation, "validation error text", true, errors.New("underlying failure"))
...
func TestPlatformLoggerEnrichment(t *testing.T) {
	...
	enrichCtx = rtcontext.WithExecution(enrichCtx, "exec-789", 5*time.Second, rtcontext.PriorityClassHigh)
