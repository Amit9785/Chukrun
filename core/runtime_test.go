...
type mockProvider struct {
	name         string
	executeFunc  func(ctx context.Context, request *execution.ExecutionRequest) (*execution.ExecutionResult, error)
	capabilities execution.ProviderCapabilities
}

func (m *mockProvider) Execute(ctx context.Context, request *execution.ExecutionRequest) (*execution.ExecutionResult, error) {
...
func (m *mockProvider) Stream(ctx context.Context, request *execution.ExecutionRequest) (<-chan execution.StreamChunk, error) {
	out := make(chan execution.StreamChunk, 2)
	out <- execution.StreamChunk{ID: request.ID, Content: "mock chunk 1"}
	out <- execution.StreamChunk{ID: request.ID, Content: "mock chunk 2"}
	close(out)
	return out, nil
}
...
func (m *mockProvider) Capabilities() execution.ProviderCapabilities {
...
func TestCoreRuntimeLifecycleAndExecution(t *testing.T) {
	...
	rt := NewRuntime(
		WithProvider(p),
		WithConfigOverrides(&config.Config{
	...
	if h.Overall != lifecycle.HealthHealthy && h.Components["lifecycle"].Details != "UNINITIALIZED" {
	...
	_, err = rt.Execute(ctx, &execution.ExecutionRequest{ID: "req-1"})
	if err == nil {
	...
	if h.Overall != lifecycle.HealthHealthy || h.Components["lifecycle"].Details != "READY" {
		t.Errorf("expected health state READY, got overall %s, state %s", h.Overall, h.Components["lifecycle"].Details)
	}

	// Execute successfully
	res, err := rt.Execute(ctx, &execution.ExecutionRequest{
		ID:          "req-2",
	...
	if res.Status != execution.StatusSucceeded || res.Output != "mock response" {
	...
	_, err = rt.Execute(ctx, &execution.ExecutionRequest{ID: "req-3"})
	if err == nil {
...
func TestGracefulShutdownAndDrain(t *testing.T) {
	...
	p := &mockProvider{
		...
		executeFunc: func(ctx context.Context, request *execution.ExecutionRequest) (*execution.ExecutionResult, error) {
			...
			case <-time.After(200 * time.Millisecond):
				return &execution.ExecutionResult{ID: request.ID, Status: execution.StatusSucceeded, Output: "slow done"}, nil
			case <-ctx.Done():
				return nil, ctx.Err()
			}
		},
	...
	var slowExecResult *execution.ExecutionResult
	...
	go func() {
		defer execWg.Done()
		slowExecResult, slowExecErr = rt.Execute(ctx, &execution.ExecutionRequest{
			ID:          "slow-req",
...
func TestConcurrencyLimiting(t *testing.T) {
	...
	p := &mockProvider{
		...
		executeFunc: func(ctx context.Context, request *execution.ExecutionRequest) (*execution.ExecutionResult, error) {
			...
			<-blockChan
			return &execution.ExecutionResult{ID: request.ID, Status: execution.StatusSucceeded}, nil
		},
	}
	...
	go func() {
		_, _ = rt.Execute(ctx, &execution.ExecutionRequest{ID: "req-active", ProviderRef: "blocking-llm"})
	}()
	...
	go func() {
		_, err := rt.Execute(ctx, &execution.ExecutionRequest{ID: "req-queued", ProviderRef: "blocking-llm"})
		queuedExecErr <- err
	}()
	...
	_, err3 := rt.Execute(ctx, &execution.ExecutionRequest{ID: "req-saturated", ProviderRef: "blocking-llm"})
	if err3 == nil {
		t.Fatal("expected request 3 to fail fast due to saturation")
	}

	platErr, ok := err3.(*rtErrors.PlatformError)
	if !ok || platErr.Category != rtErrors.ErrCategorySaturation {
...
func (m *mockMiddlewareForAdd) Dependencies() []middleware.Capability { return nil }
func (m *mockMiddlewareForAdd) Provides() []middleware.Capability     { return nil }
func (m *mockMiddlewareForAdd) FailureMode() middleware.FailureMode   { return middleware.FailClosed }

func (m *mockMiddlewareForAdd) Handle(ctx context.Context, req *execution.ExecutionRequest, next execution.Handler) (*execution.ExecutionResult, error) {
	return next(ctx, req)
}

func TestBootstrapOptions(t *testing.T) {
	log := telemetry.NewJSONLogger("debug")
...
func TestStreamingFlow(t *testing.T) {
	...
	ch, err := rt.Stream(ctx, &execution.ExecutionRequest{
		ID:          "stream-req",
...
func TestAutomaticTransitionToRunning(t *testing.T) {
	...
	h, _ := rt.Health(ctx)
	if h.State != lifecycle.StateReady {
		t.Errorf("expected state READY, got: %s", h.State)
	}

	// First execution
	_, err := rt.Execute(ctx, &execution.ExecutionRequest{
		ID:          "exec-1",
		ProviderRef: "mock-llm",
	})
	if err != nil {
...
func TestHealthCheckingWithDegradedComponent(t *testing.T) {
	...
	// Check Health
	h, _ := rt.Health(ctx)
	if h.State != lifecycle.StateDegraded || h.Overall != lifecycle.HealthDegraded {
...
func TestRestartFromFailedState(t *testing.T) {
	...
	h, _ := rt.Health(ctx)
	if h.State != lifecycle.StateFailed || h.Overall != lifecycle.HealthUnhealthy {
...
func TestAsyncExecutionEngine(t *testing.T) {
	...
	exec, err := mgr.Submit(ctx, &execution.ExecutionRequest{
		ProviderRef: "mock-llm",
	...
	if exec.State != execution.ExecStateQueued && exec.State != execution.ExecStateRunning {
	...
	if res.State != execution.ExecStateSucceeded {
...
func TestExecutionEngineCancellation(t *testing.T) {
	...
	exec1, _ := mgr.Submit(ctx, &execution.ExecutionRequest{
		ProviderRef: "mock-llm",
	})

	// Second execution goes to queue
	exec2, _ := mgr.Submit(ctx, &execution.ExecutionRequest{
		ProviderRef: "mock-llm",
	})
	...
	if res.State != execution.ExecStateCancelled {
...
func TestExecutionEngineBatching(t *testing.T) {
	...
	reqs := []*execution.ExecutionRequest{
		{ProviderRef: "mock-llm"},
		{ProviderRef: "mock-llm"},
		{ProviderRef: "mock-llm"},
	}
...
func TestCoreRuntimeMiddlewareBootstrapValidationError(t *testing.T) {
	...
	if !errors.Is(err, middleware.ErrUnsatisfiedDependency) {
...
func TestCoreRuntimeMiddlewareIntegration(t *testing.T) {
	...
	authMw := middleware.NewAuthenticationMiddleware(testVerifier)
	authzMw := middleware.NewAuthorizationMiddleware()
	cachingMw := middleware.NewCachingMiddleware()
	...
	res1, err := rt.Execute(ctx, &execution.ExecutionRequest{
		ID:          "req-unauth",
		ProviderRef: "mock-llm",
	})
	if err != nil {
	...
	if res1.Status != execution.StatusFailed || res1.Error.Category != string(rtErrors.ErrCategoryAuth) {
		t.Errorf("expected auth failure, got status %s, error %v", res1.Status, res1.Error)
	}

	// 2. Authenticated request -> should succeed
	res2, err := rt.Execute(ctx, &execution.ExecutionRequest{
		ID:          "req-auth",
		ProviderRef: "mock-llm",
	...
	if res2.Status != execution.StatusSucceeded {
