...
func TestExecutionDerivation(t *testing.T) {
	...
	execCtx := WithExecution(userCtx, "exec-1", 5*time.Second, PriorityClassHigh)
...
func TestDeadlineClamping(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			parentCtx := WithExecution(root, "parent", tt.parentDuration, PriorityClassNormal)
...
func TestCostBudgetAtomicUpdating(t *testing.T) {
	...
	ctx1 := WithCostBudget(WithExecution(root, "exec-1", 0, PriorityClassNormal), *cb)
...
func TestContextProtoSerialization(t *testing.T) {
	...
	priorities := []PriorityClass{
		PriorityClassCritical,
		PriorityClassHigh,
		PriorityClassNormal,
		PriorityClassLow,
		PriorityClassBackground,
	}
...
func TestContextConcurrencySafety(t *testing.T) {
	...
	for i := 0; i < workers; i++ {
		...
		go func(id int) {
			...
			ctx = WithExecution(ctx, "exec-con", 10*time.Second, PriorityClassNormal)
			for j := 0; j < 10; j++ {
...
func TestNativeCompatibility(t *testing.T) {
	root := NewRootContext("test-inst", "v1.0")
	ctx := WithExecution(WithSession(root, "sess-std", "user-std"), "exec-std", 0, PriorityClassNormal)
...
func BenchmarkContextDerivation(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		ctx = WithExecution(ctx, "exec", 5*time.Second, PriorityClassNormal)
		_, _ = WithMetadata(ctx, "k", "v")
	}
}
...
func TestContextMergeAdvanced(t *testing.T) {
	...
	req1 := WithExecution(root, "exec-1", 5*time.Second, PriorityClassHigh)
	req2 := WithExecution(root, "exec-2", 10*time.Second, PriorityClassNormal)
